...
@endphp
@if ($subscription->plan_type_id == 0)
<div class="subscription-status gap-x-2">سوف تنتهي باقتك بعد {{ Date::parse($subscription->expire_at)->diffForHumans()
    }}
    <a href="/customer/subscription" class="underline mx-4">تجديد</a>
</div>
@endif
<x-filament-panels::layout.base :livewire="$livewire">
    <div class="fi-layout flex min-h-screen w-full overflow-x-clip" subscription="{{ $subscription->plan_type_id }}">
        ...
        <div @if (filament()->isSidebarCollapsibleOnDesktop())
            ...
            x-bind:style="'display: flex'" {{-- Mimics `x-cloak`, as using `x-cloak` causes visual issues with chart
            widgets --}}
            ...
            'flex lg:ps-[--sidebar-width]' => ! (filament()->isSidebarCollapsibleOnDesktop() ||
            filament()->isSidebarFullyCollapsibleOnDesktop() || filament()->hasTopNavigation()),
            ])
            >
            <x-filament-panels::topbar :navigation="$navigation" />

            <main @class([ 'fi-main mx-auto w-full px-4 md:px-6 lg:px-8' , match ($maxContentWidth ?? filament()->
                getMaxContentWidth() ?? '7xl') {
                'xl' => 'max-w-xl',
                '2xl' => 'max-w-2xl',
                '3xl' => 'max-w-3xl',
                '4xl' => 'max-w-4xl',
